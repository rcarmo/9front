...
static int firstschedctxdump;
static int pid1pexitdump;
...
kenter(Ureg *ureg)
{
	...
	if(user){
		...
		if(rem < 256){
			/*
			 * During very early A733 bring-up we are still debugging first traps
			 * taken with no process context (up == nil). Let slightly-low-stack
			 * bootstrap traps proceed so trap() can report the real exception type
			 * instead of stopping immediately in kenter(). Keep the hard panic for
			 * truly tiny remaining stack or any normal post-bootstrap case.
			 */
			if(up == nil && rem >= 64)
				iprint("kenter bootstrap low stack: %d bytes left, ureg %#p pc %#p type %#p\n",
					rem, ureg, ureg->pc, ureg->type);
			else
...
kexit(Ureg*)
{
	...
	/*
	 * Temporary bring-up bypass: touching the user TOS page on the very first
	 * EL0 fault-return path appears to be the next failing edge after pid1's
	 * stack-write fault is resolved. Skip the TOS accounting for pid 1 so we can
	 * see whether first user syscalls start working.
	 */
	if(up != nil && up->pid == 1)
		return;
...
sched(void)
{
	...
	if(firstschedctxdump == 0){
		firstschedctxdump = 1;
		iprint("sched handoff pid %lud text %s schedpc %#p schedsp %#p linkproc %#p kpfun %#p\n",
			up->pid, up->text, up->sched.pc, up->sched.sp, linkproc, up->kpfun);
		procmark('}');
	}
	gotolabel(&up->sched);
}
...
pexit(char *exitstr, int freemem)
{
	...
	if(up != nil && up->pid == 1 && pid1pexitdump < 8){
		pid1pexitdump++;
		iprint("pid1 pexit[%d] text %s exit %s scallnr %d nnote %d procctl %d pc %#p sp %#p\n",
			pid1pexitdump-1,
			up->text != nil ? up->text : "?",
			exitstr != nil ? exitstr : "",
			up->scallnr, up->nnote, up->procctl,
			up->dbgreg != nil ? up->dbgreg->pc : 0,
			up->dbgreg != nil ? up->dbgreg->sp : 0);
		if(up->lastnote != nil)
			iprint("pid1 pexit note %s flag %d\n", up->lastnote->msg, up->lastnote->flag);
	}
	up->alarm = 0;
	timerdel(up);
	pt = proctrace;
