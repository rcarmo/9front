...
static int firstkprocchilddump;
static int pid1syscalldump;
static int pid1faultdump;
static int pid1trapdump;
static int pid1irqdump;
static int pid1frameafterfault;
static int pid1framebeforeexit;
static int pid1stackwalkdump;

static void
dumppid1stackwalk(uintptr addr)
{
	Page *t;
	Segment *s;
	Pte *ptab;
...
void
trapresume(uintptr tag, uintptr elr, uintptr sp, uintptr spsr, uintptr type)
{
	USED(tag, elr, sp, spsr, type);
}
...
trap(Ureg *ureg)
{
	...
	if(up == nil){
		iprint("bootstrap trap: intr %#ux type %#ux far %#p pc %#p sp %#p link %#p\n",
			intr, type, getfar(), ureg->pc, ureg->sp, ureg->link);
		dumpregs(ureg);
		panic("bootstrap trap");
	}
	user = kenter(ureg);
	switch(type){
	...
	case 0x25:	// data abort from same level
		fpukenter(ureg);
		faultarm64(ureg);
		if(user && up != nil && up->pid == 1 && pid1frameafterfault++ == 0)
			iprint("pid1 ureg postfault pc %#p sp %#p psr %#p type %#p link %#p\n",
				ureg->pc, ureg->sp, ureg->psr, ureg->type, ureg->link);
		break;
	case 0x07:	// SIMD/FP
	...
	case 0x00:	// unknown
		if(intr == 1){
			fpukenter(ureg);
			if(user && up != nil && up->pid == 1 && pid1irqdump < 1)
				iprint("pid1 irq[%d] pc %#p sp %#p link %#p\n",
					pid1irqdump++, ureg->pc, ureg->sp, ureg->link);
			/*
			 * Temporary bring-up bypass: once pid1 reaches EL0, timer IRQs start
			 * preempting it before the first syscall becomes visible. Service the
			 * interrupt but skip scheduler-preemption until pid1 has reached its
			 * first syscall entry.
			 */
			if(user && up != nil && up->pid == 1 && pid1syscalldump == 0)
				irq(ureg);
			else
				preempted(irq(ureg));
			break;
	...
	default:
		...
		if(up != nil && up->pid == 1 && pid1trapdump < 1)
			iprint("pid1 trap[%d] intr %#ux type %#ux pc %#p sp %#p far %#p link %#p\n",
				pid1trapdump++, intr, type, ureg->pc, ureg->sp, getfar(), ureg->link);
	...
	splhi();
	if(user){
		if(up != nil && up->pid == 1 && pid1syscalldump == 0)
			ureg->psr |= 1<<7;	/* mask IRQs until first syscall */
		if(up->procctl || up->nnote)
			donotify(ureg);
		if(up != nil && up->pid == 1 && pid1framebeforeexit++ == 0)
			iprint("pid1 ureg preexit pc %#p sp %#p psr %#p type %#p link %#p\n",
				ureg->pc, ureg->sp, ureg->psr, ureg->type, ureg->link);
		kexit(ureg);
	}
...
syscall(Ureg *ureg)
{
	...
	scallnr = ureg->r0;
	/* first pid1 syscall reached: allow normal IRQ delivery again */
	if(up != nil && up->pid == 1)
		ureg->psr &= ~(1<<7);
	tracedump = -1;
	if(up != nil && up->pid == 1 && pid1syscalldump < 4){
		tracedump = pid1syscalldump++;
		name = scallnr < nsyscall && sysctab[scallnr] != nil ? sysctab[scallnr] : "?";
		iprint("pid1 syscall[%d] %lud %s pc %#p sp %#p args %#p\n",
			tracedump, scallnr, name, ureg->pc, ureg->sp, ureg->sp+BY2WD);
	}
	donote = dosyscall(scallnr, (Sargs*)(ureg->sp+BY2WD), &ureg->r0);
	if(tracedump >= 0)
		iprint("pid1 syscall[%d] ret %#p%s\n", tracedump, ureg->r0, donote ? " noted" : "");
	if(donote)
		returnto(noteret);
...
faultarm64(Ureg *ureg)
{
	...
	tracedump = -1;
	if(user && up != nil && up->pid == 1 && pid1faultdump < 1){
		tracedump = pid1faultdump++;
		iprint("pid1 fault[%d] esr %#ux pc %#p far %#p %s\n",
			tracedump, (u32int)ureg->type & 0x3F, ureg->pc, addr, read ? "read" : "write");
	}
	...
	case 48:				// tlb conflict fault.
		if(fault(addr, ureg->pc, read) == 0){
			if(tracedump >= 0)
				iprint("pid1 fault[%d] resolved\n", tracedump);
			if(user && up != nil && up->pid == 1 && addr >= USTKTOP-BY2PG && addr < USTKTOP)
				dumppid1stackwalk(addr);
			break;
		}
...
kprocchild(Proc *p, void (*entry)(void))
{
	...
	p->sched.sp = ((uintptr)p - 16) & ~0xFULL;
	*(void**)p->sched.sp = kprocchild;	/* fake */
	if(firstkprocchilddump == 0){
		firstkprocchilddump = 1;
		iprint("kprocchild p %#p entry %#p schedpc %#p schedsp %#p fakelr %#p\n",
			p, entry, p->sched.pc, p->sched.sp, *(uintptr*)p->sched.sp);
	}
}
...
forkchild(Proc *p, Ureg *ureg)
{
	...
	p->sched.sp = ((uintptr)p - TRAPFRAMESIZE) & ~0xFULL;
