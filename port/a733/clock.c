...
{
	/*
	 * During A733 bring-up we temporarily shut off the virtual timer source
	 * itself, not just DAIF IRQ delivery, so late bootstrap cannot re-enter on
	 * the mach stack before the scheduler is ready.
	 */
	syswr(CNTV_CTL_EL0, 0);
}
